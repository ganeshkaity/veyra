...
const gifRateLimitMap = new Map<string, { count: number; resetAt: number }>();
const MAX_GIF_REQUESTS_PER_MINUTE = 60;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const now = Date.now();
    const rateRecord = gifRateLimitMap.get(clientIp);

    if (rateRecord && now < rateRecord.resetAt) {
      if (rateRecord.count >= MAX_GIF_REQUESTS_PER_MINUTE) {
        const retryAfterSec = Math.ceil((rateRecord.resetAt - now) / 1000);
        return NextResponse.json(
          { error: `Too many GIF search requests. Please wait ${retryAfterSec} seconds.` },
          {
            status: 429,
      ...
      gifRateLimitMap.set(clientIp, { count: 1, resetAt: now + WINDOW_MS });
    }
    ...
    const endpoint = rawQuery
      ...
      : `https://api.giphy.com/v1/gifs/trending?api_key=${apiKey}&limit=${safeLimit}&offset=${safeOffset}&rating=g`;
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: data?.message || "Failed to fetch GIFs from GIPHY" },
        { status: res.status }
      );
    }
    ...
    return NextResponse.json({ success: true, gifs: results });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Server error fetching GIFs";
